...
export default function Hero({ scroll }) {
  ...
  useEffect(() => {
    if (
      hero.current.offsetTop + hero.current.clientHeight * 0.4 >= scroll &&
      document.querySelector("html").style.background !==
        "rgba(241, 223, 213, 0.5)"
    ) {
      document.querySelector("html").style.background =
        "rgba(241, 223, 213, 0.5)";
      veilNoise.current.style.opacity = 1;
      document.getElementsByClassName("primary-cursor")[0].style.background =
        "linear-gradient(to bottom right, transparent, #3398ff 80%)";
    } else if (
      hero.current.offsetTop + hero.current.clientHeight * 0.4 < scroll &&
      hero.current.offsetTop + hero.current.clientHeight * 0.5 >= scroll &&
      ...
      >
        <div
          ...
          style={{
            ...
            textAlign: "center",
            height: "200%",
            animation: "translateYAnimation 5s infinite",
          }}
        >
          <div style={{ height: "50%", fontWeight: "300" }}>
            {words[counter]}
          </div>
          <div style={{ height: "50%", fontWeight: "300" }}>
            {words[counter + 1]}

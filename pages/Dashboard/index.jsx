...
const Dashboard = () => {
  ...
  if (monthlyUsers) {
    monthlyUsers.forEach(user => {
      const month = new Date(user.data.created_at.seconds * 1000).toLocaleString('default', { month: 'long' });
  ...
  const addNewBlog = async () => {
    ...
    try {
      const response = await axios.post('http://localhost:3000/api/blogs/add', blog);
  ...
  const deleteBlog = async (id) => {
    try {
      const response = await axios.delete(`http://localhost:3000/api/blogs/delete/${id}`)
  ...
  const updateBlog = async (id) => {
    ...
    try {
      const response = await axios.put(`http://localhost:3000/api/blogs/update/blog/${id}`, updatedData)
      console.log(response)
    }
    catch (error) {
          ...
          <tbody>
            {blogs && blogs.map((blog, index) => (
              <tr key={index}>
                ...
                <td > <img style={{ width: "100px", height: "50px" }} src={blog.data.image} alt="" />   </td>

                <td >{blog.data.created_at}</td>
                <td>
                  <button type="button" onClick={() => deleteBlog(blog.id)}>delete</button>
                  <button type="button" onClick={() => updateBlog(blog.id)}>update</button>
                </td>
              </tr>

            ))}
            <tr>
              <td><input type='text' placeholder='blog title' onChange={(e) => setBlogTitle(e.target.value)}></input></td>
              <td><input type='text' placeholder='description' onChange={(e) => setDescription(e.target.value)} ></input></td>
              <td><WidgetLoader />
                <Widget
                  sources={['local', 'camera', 'dropbox', 'instagram', 'facebook', "google_drive"]} // 
                  resourceType={'image'}
                  cloudName={'dbea96pnt'}
              ...
              <td><button type="button" onClick={() => addNewBlog()} >Add New Blog</button></td>
            </tr>
          </tbody>
        </table>
      </div>
